...
# Business meaning - If we use the log.prediction with 10% cutoff, we would get: 297*383 - (1908+383)*48 = 113,751-109,968 = 3783$ 
# If we use the log.prediction with 20% cutoff, we would get: 198*297 - (491+198)*48 = 58,806-33,072 = 25,734$

# Try creating new data containing only significant values 
...
log.prediction.rd <- ifelse(log.predictions > 0.2, 1, 0) # Different cutoffs gives different accuracy measure 
### Evaluation of the model using confusion matrix 
table(log.prediction.rd, ffp_test__new[,17])

# Accuracy 
accuracy <- table(log.prediction.rd, ffp_test__new[,17])
sum(diag(accuracy))/sum(accuracy) # Accuracy = 88%

# Using less variables didn't change log model accuracy 
